...
// 3. Создаем ViewPoint, передавая ему canvas для отслеживания событий
const viewPoint = new ViewPoint({
    element: renderer.canvas!,
    fov: (2 * Math.PI) / 5,       // Угол обзора
    aspect: window.innerWidth / window.innerHeight, // Соотношение сторон
})

// 4. Используем viewPoint в цикле рендеринга
function animate() {
    // ViewPoint сам обновляется при вводе пользователя.
    // Вызывать update() вручную НЕ НУЖНО.
