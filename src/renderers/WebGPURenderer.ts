...
// struct SceneUniforms { viewMatrix: mat4x4f, viewNormalMatrix: mat4x4f, numLights: u32, --padding--, lights: array<Light, 4> }
// viewMatrix: 64 байта
// viewNormalMatrix: 64 байта
// numLights (u32): 4 байта.
// Отступ для выравнивания массива lights до 16 байт: 12 байт.
// Итого: 64 + 64 + 4 + 12 + 4 * 32 = 272 байта.
const SCENE_UNIFORMS_SIZE = 272;
const LIGHT_STRUCT_SIZE = 32; // Размер структуры Light в байтах
...
export class WebGPURenderer {
  ...
  private updateSceneUniforms(lights: LightItem[], viewMatrix: Matrix4): void {
    ...
    // Записываем viewMatrix (смещение 0)
    float32View.set(viewMatrix.elements, 0);
    // Записываем viewNormalMatrix (смещение 64 байта)
    float32View.set(viewNormalMatrix.elements, 16);

    // Записываем количество источников света (смещение 128 байт)
    uint32View[32] = Math.min(lights.length, MAX_LIGHTS);

    // Смещение массива lights в буфере: 144 байта (36 * 4 байта)
  ...
  private getOrCreateGeometryBuffers(
    ...
    if (geometry.index) {
      const TypedArray =
        geometry.index.array.length > 65535 ? Uint32Array : Uint16Array;
  ...
  private getLineShaderCode(): string {
    return `
      ...
      // Эта структура ДОЛЖНА в точности повторять ту, что в mesh shader,
      // так как они используют общий "чертеж" (bind group layout).
      struct Light {
        position: vec4<f32>,
        color: vec4<f32>,
      };
      struct SceneUniforms {
          viewMatrix: mat4x4<f32>,
          viewNormalMatrix: mat4x4<f32>, // <--- ДОБАВЛЕНО ДЛЯ СИНХРОНИЗАЦИИ
          numLights: u32,
          lights: array<Light, ${MAX_LIGHTS}>,
      };
      @binding(1) @group(0) var<uniform> sceneUniforms: SceneUniforms;
